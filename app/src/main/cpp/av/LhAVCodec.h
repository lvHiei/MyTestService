...
public:
    ...
    int init_video_encoder(AVCodecID codecID, int width, int height, int framerate, int bitrate, int gop, AVDictionary** param);
    int init_audio_encoder(AVCodecID codecID, int samplerate, int channels, int bitrate, int profile);
...
private:
    AVCodecContext* m_pEncoderCodecContext;
    AVCodecContext* m_pDecoderCodecContext;
